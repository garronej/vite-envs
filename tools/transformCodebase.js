...
function transformCodebase(params) {
    ...
    try {
        for (var _c = __values(crawl_1.crawl(srcDirPath)), _d = _c.next(); !_d.done; _d = _c.next()) {
            ...
            var newFileBasename = transformSourceCodeResult.newFileBasename, modifiedSourceCode = transformSourceCodeResult.modifiedSourceCode;
            fs.writeFileSync(path.join(path.dirname(path.join(destDirPath, file_relative_path)), newFileBasename !== null && newFileBasename !== void 0 ? newFileBasename : path.basename(file_relative_path)), modifiedSourceCode);
        }
    }
