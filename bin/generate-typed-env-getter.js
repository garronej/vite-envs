...
fs.writeFileSync(path_1.join(targetProjectDirPath, "src", "env.ts"), Buffer.from([
    "// Generated by " + path_1.basename(__filename).split(".")[0] + ". Not meant to be edited manually",
    "import { getEnvVarName\u00A0}\u00A0from \"" + require(path_1.join(getProjectRoot_1.getProjectRoot(), "package.json"))["name"] + "\";",
    '',
    'export function getEnv() {',
    '    return {',
    getEnvNames_1.getEnvNames({ targetProjectDirPath: targetProjectDirPath })
        .map(function (envName) { return "        \"" + envName + "\": getEnvVarName(\"" + envName + "\")"; })
        .join(",\n"),
    '    } as const;',
    '}',
    ''
